...
public:
    ...
    // not implemented because it is not beneficial, 
    // Usefulness of this class will be decided at the end of the project.
    // void coutRegister() {  }

    void coutEnterRange(int start, int end) {
...
public:
    ...
    void GetQuestions(vector<QuestionDetails> &questions, int user_id, int col) {
        ...
        while(getline(ReadThis, record)) {
            ...
            // This is a very bad design I admit.
            if(user_id == stoi(db_questiondetails[col]) || user_id == -10) { // -10 => get me everything
...
public:
    ...
    void AskQuestion() {
        // From here
        // I think can be replaced by QuestionDetails
        int asked_uid;
        ...
        if(asked_userdetails.uid == -1) {
            cout << "This uesr id does not exist!\n";
            AskQuestion(); // But why?
            return;
        }
        ...
        // To here -> Public function, rest is private function.
        string question;
        ...
        // QuestionID - AskerID - AskedID - AskerName - AskedName - QuestionData - AnswerData
        // No getters... AND REVERSE OREDER IN HRERE #here
        // isAQ Feature edit 1
        QuestionDetails newQuestion (qid, user_id, asked_userdetails.uid, question, "", isAQ);
        ReadThis << newQuestion.question_id << '\t' << newQuestion.asker_id << '\t' 
                 << newQuestion.asked_id << '\t' << newQuestion.question_data << '\t' << newQuestion.is_aq << '\n';
...
private:
    ...
    void Login() {
        ...
        // I don't like that I am parsing this in Login(),
        // but if I don't either I move is_username & is_password to another function, which I think defies the point
        // or make "that function" return parsed DBusername & DBpassword which is... meh.
        fstream ReadThis; // #repeated2
        ReadThis.open("users.csv", fstream::in);
        string record, word;
        ...
        while(getline(ReadThis, record)) {
            ...
            if(is_username == db_userdetails[3] && is_password == db_userdetails[4]) {
                user_id = stoi(db_userdetails[0]);
                anon_q = db_userdetails[5][0] - '0'; // <<<
                ReadThis.close();
                return;
            }
        }
    ...
    void Register() {
        ...
        user_id = user_details.uid = IncrementUsers() - 1;
        anon_q = user_details.anonymous;
    ...
    void AddToDatabse(UserDetails user_details) {
        fstream ReadFile;
    ...
    User() {
        ...
        while(choice != -1) {
            ...
            if(choice == 1)
                ...
                PrinterObj.coutError();
                PrinterObj.coutEnterRange(1, 2);
            } 
        }
    }
